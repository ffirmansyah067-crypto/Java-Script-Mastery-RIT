...
// FORM F3: Input Nama & Character Playable
document.getElementById("F3").addEventListener("submit", function (e) {
    e.preventDefault();
    
    const nama = document.getElementById("namaF3").value;
    const character= document.getElementById("characterF3").value;

    if (!nama || !character) {
        alert("Masukkan Nama dan Pilih Karakter Kamu!");
        return;
    }
...
function f3(nama, character) {
    const output = document.getElementById("outputBox");
    output.innerText += `F3: Nama: ${nama}, Charactermu: ${character}\n`;
    togglePopup("3"); // Tutup modal setelah selesai
    return { nama, character };
}
...
function f10_trigger() {
    const result = exspresi("Firmansyah", "Depok");
    const output = document.getElementById("outputBox");
